  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        {/* Logo Icon - Moves to top left on scroll */}
        <motion.div
          initial={{ scale: 0, opacity: 0 }}
          animate={{ 
            scale: scrolled ? 0.5 : 1, 
            opacity: 1,
            x: scrolled ? -window.innerWidth * 0.4 : 0,
            y: scrolled ? -window.innerHeight * 0.4 : 0
          }}
          transition={{ duration: 0.6, ease: "easeInOut" }}
          className={`${scrolled ? 'fixed top-6 left-6 z-30' : 'absolute'}`}
        >
          <Link href="/" onClick={scrollToTop}>
            <motion.div
              whileHover={{ scale: 1.05, rotate: 5 }}
              whileTap={{ scale: 0.95 }}
              className="w-32 h-32 md:w-40 md:h-40 rounded-3xl bg-card shadow-hover flex items-center justify-center cursor-pointer group"
            >
              <div className="relative">
                <Code2 className="w-16 h-16 md:w-20 md:h-20 text-primary group-hover:text-accent transition-colors" />
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto space-y-8">
          ...
          <motion.p
            ...
            className="text-base md:text-lg lg:text-xl text-foreground max-w-3xl mx-auto"
          >
      ...
      >
        <Link href="/apis" className="text-sm font-semibold text-foreground hover:text-primary transition-colors">
          Browse APIs
        </Link>
